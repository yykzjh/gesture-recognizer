...
#     #线程运行函数
#     def run(self):
        # gestureDataSrc = pd.read_excel(r"./手势数据/圆正斜圆正斜矩正斜圆.xlsx",usecols="B:G",columns=['x','y','z','a','b','c']) #手势数据的集合形式
        # ind = 0
        # global gestureData
        # while ind<gestureDataSrc.shape[0]:
        #     if (ind+20)<gestureDataSrc.shape[0]:
        #         gestureDataSlice = gestureDataSrc.iloc[ind:ind+20,:]
        #         gestureDataSlice.columns = ['x','y','z','a','b','c']
...
class Recognizer(QWidget):
    ...
    def initUI(self):
        ...
        self.table1.setStyleSheet("font-size:16px;font-family:Arial;text-align:center;")
        ...
        self.label8.setMaximumHeight(300) #设置图片最大显示高度
        self.label8.setAlignment(Qt.AlignHCenter) #设置显示图片的标签水平居中
        self.label8.setScaledContents (True) #设置图片自适应标签大小
        self.label8.setStyleSheet("border:1px solid black;margin:0 250;"); #设置显示手势图片标签的样式
        ...
        self.table2.setStyleSheet("font-size:20px;font-weight:500;font-family:Arial;")
        ...
        rightBoxLayout.addWidget(self.table2) #将历史记录框添加到布局
    ...
    def updateUI(self):
        ...
        for i in range(gestureDataTmp.shape[0]):
            self.table1.setRowHeight(i, 50)
            newItem = QTableWidgetItem(str(i))
            newItem.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.table1.setItem(i, 0, newItem)
        ...
        self.table2.setRowCount(math.ceil(len(historyResults)/6))
        for i in range(len(historyResults)):
            if (i%6)==0:
                self.table2.setRowHeight(int(i/6), 50)
            newItem = QTableWidgetItem(str(historyResults[i]))
            newItem.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
            self.table2.setItem(int(i/6), i%6, newItem) # 设置表格内容(行,列,文本)
...
gestureData = pd.DataFrame(columns=['ax','ay','az'])
recognize_is_start = False
startIndex = 0
...
if __name__ == '__main__':
    ...
    global t2
    t2 = threading.Thread(target=deal_data)
    t2.setDaemon(True)
