...
import threading
import pandas as pd
from Segment import cal_resistance_data, segment_resistance_data
from time import sleep
from Recognizer import Dtw, match_distance
...
def cal_data(data):
    ...
    for i in range(0,data.shape[1],2):
        I = (3.3 - data[:,i]) / 200000 #计算电流
        data[:,i] = (data[:,i] - data[:,i+1]) / I
        data[:,i+1] = data[:,i+1] / I
    ...
    for i in range(data.shape[1]):
        data[:,i] = np.clip(data[:,i],bottomLimit[i],topLimit[i])
        data[:,i] = (data[:,i] - bottomLimit[i]) / (topLimit[i] - bottomLimit[i])
...
def read_data():
    ...
    #初始化
    global device_is_open
    device_is_open = True

    #调用获取采集卡数据的接口
    cur_path = os.path.dirname(__file__)
    dllPath = os.path.join(cur_path,"ADIO64.dll")
    pDll = CDLL(dllPath)

    # #找到设备
    # pDll.FindDevice()
    # #打开设备
    # pDll.OpenYavDevice(0)

    # #设定采样率
    ParamList = (c_char*4)(0x03,0,0,0)
    pDll.SetYavParam(0,0xFA,ParamList)

    #设置获取数据的参数
    # DAQDataBuffer = (c_int*256)()
    # YavParam = (c_int*4)()
    # CNTBuffer = (c_int*2)()
    # IOBuffer = (c_int*2)()
    # pDll.GetYavData(0,DAQDataBuffer,64,YavParam,CNTBuffer,IOBuffer) #获取64个数据

    #定义4行取1行计数器
    cnt = 0

    while device_is_open:
        #获取数据
        DAQDataBuffer = (c_int*64)() #定义获取数据的缓存区
        pDll.GetYavOnce(0,DAQDataBuffer) #获取64个数据
        ...
        for i in range(4,28,3):
            onceDealData.append((DAQDataBuffer[i]<<4)+(DAQDataBuffer[i+1]>>4))
            onceDealData.append(((DAQDataBuffer[i+1]&15)<<8)+DAQDataBuffer[i+2])
        
        #将数据变为np数组
        onceDealDataArray = np.array(onceDealData).reshape(1,16)

        #第二次处理数据
        gestureNewDataArray = onceDealDataArray / 4095 * 5
        selectCols = [1,2,4,5,8,9,11,12,14,15] #定义需要选择的10个通道
        gestureNewDataArray = gestureNewDataArray[:,selectCols] #选择需要的10个通道的数据

        gestureNewDataArray = cal_data(gestureNewDataArray) #根据电压数据计算电阻归一化数据
        gestureNewDataPd = pd.DataFrame(gestureNewDataArray, columns=['通道1','通道4','通道5','通道8','通道9','通道11','通道12','通道14','通道15'])
        gestureData = pd.concat([gestureData, gestureNewDataPd]) #将新增数据添加到总数据集合
...
def read_check_data():
    #定义保存全局的校正数据集
    global checkData 
    checkData = pd.DataFrame(columns=['通道1','通道2','通道4','通道5','通道8','通道9','通道11','通道12','通道14','通道15'])

    #初始化
    global check_is_open
    check_is_open = True
    ...
    while check_is_open:
        ...
        #转化为dataframe格式
        gestureNewDataPd = pd.DataFrame(gestureNewDataArray,columns=['通道1','通道2','通道4','通道5','通道8','通道9','通道11','通道12','通道14','通道15'])

        #合并到总数据集合
        checkData = checkData.append(gestureNewDataPd, ignore_index=True)
...
class WorkThread(QThread):
    ...
    #线程运行函数
    def run(self):
        gestureDataSrc = pd.read_excel(r"./第二次手套数据/手套1/第8组/处理后的数据集/P_手套1-8次-组合-握拳-4-0-4-点赞-握拳-点赞-0.xlsx",usecols=[2,3,5,6,9,10,12,13,15,16],header=0) #手势数据的集合形式
        ind = 0
        global gestureData
        while ind<gestureDataSrc.shape[0]:
            if (ind+20)<gestureDataSrc.shape[0]:
                gestureDataSlice = gestureDataSrc.iloc[ind:ind+20,:]
                gestureDataSlice.columns = ['通道1','通道2','通道4','通道5','通道8','通道9','通道11','通道12','通道14','通道15']
                gestureDataSlice = cal_data(gestureDataSlice)
...
class Recognizer(QWidget):
    ...
    def initUI(self):
        ...
        self.table1.setStyleSheet("font-size:16px;font-family:Arial;text-align:center;background-color:auto;")
        ...
        #第二行的功能控件
        btn2_1 = QPushButton("开始校正")
        btn2_1.setStyleSheet("font-size:16px;font-weight:550;font-family:Microsoft YaHei;background-color:white;") #按钮字体样式
        btn2_1.setMinimumHeight(50)
        btn2_1.clicked.connect(self.start_check)
        lineBoxLayout2.addWidget(btn2_1,3)
        lineBoxLayout2.addStretch(2) #添加空白占位

        btn2_2 = QPushButton("结束校正")
        btn2_2.setStyleSheet("font-size:16px;font-weight:550;font-family:Microsoft YaHei;background-color:white;") #按钮字体样式
        btn2_2.setMinimumHeight(50)
        btn2_2.clicked.connect(self.end_check)
        lineBoxLayout2.addWidget(btn2_2,3)
        lineBoxLayout2.addStretch(2) #添加空白占位
    ...
    def updateUI(self):
        ...
        self.table1.clear()
        self.table1.setRowCount(math.ceil(gestureDataTmp.shape[0])+1)
        ...
        for i in range(0,gestureDataTmp.shape[0]):
            self.table1.setRowHeight(i, 50)
            for j in range(gestureDataTmp.shape[1]):
                newItem = QTableWidgetItem(str(gestureDataTmp[i][j]))
                newItem.setTextAlignment(Qt.AlignHCenter | Qt.AlignVCenter)
                self.table1.setItem(i+1, j, newItem) # 设置表格内容(行,列,文本)
    ...
    def start_recognize(self):
        ...
        global device_is_open
        if device_is_open:
            messageBox2_1 = QMessageBox()
            messageBox2_1.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox2_1.information(self, '翻译结果', "<div style='font-size:32px;'>识别已经开始，请勿重复点击识别！</div>", QMessageBox.Ok)
            return 

        #检测USB是否正在获取校正数据
        global check_is_open
        if check_is_open:
            messageBox2_2 = QMessageBox()
            messageBox2_2.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox2_2.information(self, '翻译结果', "<div style='font-size:32px;'>获取数据接口正在获取校正数据，请先结束校正！</div>", QMessageBox.Ok)
            return 
        ...
        #打开分析识别手势线程
        t2 = threading.Thread(target=deal_static_data)
        t2.setDaemon(True)
        t2.start()
    ...
    def end_recognize(self):
        #检测是否存在正在进行的获取识别数据线程
        global device_is_open
        if not device_is_open:
            messageBox5 = QMessageBox()
            messageBox5.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox5.information(self, '翻译结果', "<div style='font-size:32px;'>您并没有开始识别！</div>", QMessageBox.Ok)
            return 
        ...
        #清空缓存数据
        global gestureData
        gestureData.drop(gestureData.index,inplace=True)
    ...
    def start_check(self):
        ...
        global check_is_open
        if check_is_open:
            messageBox3_1 = QMessageBox()
            messageBox3_1.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox3_1.information(self, '翻译结果', "<div style='font-size:32px;'>校正已经开始，请勿重复点击校正！</div>", QMessageBox.Ok)
            return 

        #检测USB是否正在获取待识别数据
        global device_is_open
        if device_is_open:
            messageBox3_2 = QMessageBox()
            messageBox3_2.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox3_2.information(self, '翻译结果', "<div style='font-size:32px;'>获取数据接口正在获取待识别数据，请先结束识别！</div>", QMessageBox.Ok)
            return 

        #打开获取校正数据的线程
        t3 = threading.Thread(target=read_check_data)
        t3.setDaemon(True)
        t3.start()
    ...
    def end_check(self):
        ...
        if not check_is_open:
            ...
            messageBox6.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox6.information(self, '翻译结果', "<div style='font-size:32px;'>您并没有开始获取校正数据！</div>", QMessageBox.Ok)
            return 
        ...
        #判断是否分割出了上下界数据
        if len(dataSlicePoint) < 2:
            messageBox4 = QMessageBox()
            messageBox4.setStyleSheet("QPushButton {"
                            "background-color:#89AFDE;"
                            " border-style: outset;"
                            " border-width: 2px;"
                            " border-radius: 10px;"
                            ...
                            "}"
            "QLabel { min-width: 20em;min-height:10em;font:24px;background-color:#f0f0f0;}")
            messageBox4.information(self, '翻译结果', "<div style='font-size:32px;'>校正数据失败！</div>", QMessageBox.Ok)
            return 
...
def deal_static_data():
    ...
    while recognize_is_start:
        ...
        fluctuation = np.zeros(gestureData.shape[0]-startIndex-FrameLength)
        for i in range(fluctuation.shape[0]):
            fluctuation[i] = np.sum(np.var(gestureData.iloc[startIndex+i:startIndex+i+FrameLength+1,:]))
        
        # 观察波动值变化情况
        # plt.plot(fluctuation)
        # plt.xticks(np.arange(0,len(fluctuation),40))
        # plt.axhline(y=0.002,c='red',ls='--',lw=1)
        # plt.axhline(y=0.003,c='red',ls='--',lw=1)
        # plt.show()
        ...
        for i in  range(0,fluctuation.shape[0],StepLength):
            if not findStart and fluctuation[i] < Threshold1:
                ...
                distance = dict() #用于保存当前待识别的手势序列和不同手势模板的距离
                for k,v in models.items(): #与不同的手势模板比较
                    pos = (sliceTmp.start+sliceTmp.end)//2
                    distance[k] = match_distance(gestureData.iloc[[sliceTmp.start,pos,sliceTmp.end],:],v) #用Dtw求手势序列和手势模板的距离
                if distance[min(distance,key=distance.get)] == float('inf'):
                    print("不存在该手势的模板") #所有模板都没匹配上
                else:
                    result = min(distance,key=distance.get)
                    historyResults.append(result)
                    pixmap2 = QPixmap("./images/results/"+result+".jpg") #读入图片
                    ex.label8.setPixmap(pixmap2)
                    ex.gestureMeans.setText(result)
...
gestureData = pd.DataFrame(columns=['通道1','通道4','通道5','通道8','通道9','通道11','通道12','通道14','通道15'])
recognize_is_start = False
startIndex = 0
...
if __name__ == '__main__':
    ...
    global models
    models = pd.read_excel(r"./手套数据/models.xlsx",sheet_name=None) #读入模板数据
